...
`}n.innerHTML=r.map(a).join(""),new f(".gallery a",{captionsData:"alt",captionDelay:250}).refresh()},v=()=>{n.innerHTML=""},b=document.querySelector(".search-form"),c=document.querySelector(".loader-placeholder");b.addEventListener("submit",r=>{r.preventDefault();const l=r.target.elements.input.value.trim();if(l===""){s("Please enter something");return}v(),c.classList.add("loader"),y(l).then(o=>{if(console.log(o),o.data.hits.length===0){s("Sorry, there are no images matching your search query. Please, try again!");return}F(o.data.hits)}).catch(o=>{console.log(o)}).finally(()=>{c.classList.remove("loader")})});
