import{a as c,S as m,i as h}from"./assets/vendor-DH2CaQaJ.js";(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))s(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const r of t.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function i(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function s(e){if(e.ep)return;e.ep=!0;const t=i(e);fetch(e.href,t)}})();const g="48865554-9c41d4f1cdb1a3126cd29aed3",f="https://pixabay.com/api/",y=a=>c.get(`${f}?key=${g}&${a}`),C="AIzaSyDkma_-23SOOFT86C4sfDZ_GCLFzM_NQao",v="https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent",b=a=>`
    <instruction>
...
`}d.innerHTML=a.map(o).join(""),F.refresh()},x=()=>{d.innerHTML=""},_=document.querySelector(".search-form"),n=document.querySelector(".loader-placeholder");_.addEventListener("submit",a=>{a.preventDefault();const i=a.target.elements.input.value.trim();if(i===""){l("Please fill out this field");return}x(),n.classList.add("loader");let s=b(i);L(s).then(e=>{let t=e.data.candidates[0].content.parts[0].text;console.log(t),y(t).then(r=>{if(console.log(r),r.data.hits.length===0){l("Sorry, there are no images matching your search query. Please, try again!");return}A(r.data.hits)}).catch(r=>{console.log(r)})}).catch(e=>{console.log(e)}).finally(()=>{n.classList.remove("loader")})});
